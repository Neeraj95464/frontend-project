...
// src/pages/EmployeeAcceptance.jsx - Updated fetchDetails and handleAccept
// import React, { useState, useEffect, useRef } from 'react';
// import { useParams, useNavigate } from 'react-router-dom';
// import { employeeItemApi } from '../services/api';
...
//   // Dress customization state
//   const [dressCustomizations, setDressCustomizations] = useState([]);
//   const [originalDressItems, setOriginalDressItems] = useState({});
...
//       // Initialize dress customizations if it's a dress item
//       if (data.itemType?.toLowerCase().includes('dress') && data.dressItems && Object.keys(data.dressItems).length > 0) {
//         setOriginalDressItems(data.dressItems);
//         const initialCustomizations = Object.entries(data.dressItems).map(([name, quantity], index) => {
//           // Parse the name to extract base name and existing size
//           let baseName = name;
//           let existingSize = '';
//           let existingColor = '';
          
//           // Check if name has size/color info in parentheses
//           const match = name.match(/^(.*?)\s*\(([^)]+)\)$/);
//           if (match) {
//             baseName = match[1];
//             const attributes = match[2].split(',');
//             existingSize = attributes[0].trim();
//             if (attributes[1]) existingColor = attributes[1].trim();
//           }
...
//   const handleRemoveDressItem = (id) => {
//     setDressCustomizations(prev => prev.filter(item => item.id !== id));
//   };

//   const handleUpdateDressItemName = (id, value) => {
//     setDressCustomizations(prev => 
//       prev.map(item => item.id === id ? { ...item, name: value } : item)
//     );
//   };

//   const handleUpdateDressItemQuantity = (id, value) => {
//     setDressCustomizations(prev => 
//       prev.map(item => item.id === id ? { ...item, quantity: parseInt(value) || 1 } : item)
//     );
//   };
...
//     // Validate based on item type
//     if (details?.itemType?.toLowerCase().includes('dress')) {
//       // Check if any item has no name
//       const missingNames = dressCustomizations.filter(item => !item.name.trim());
//       if (missingNames.length > 0) {
//         setError(`Please enter names for all dress items`);
//         return;
//       }
      
//       // Validate sizes (optional now)
//       const missingSizes = dressCustomizations.filter(item => !item.selectedSize);
//       if (missingSizes.length > 0) {
//         if (window.confirm(`You haven't selected sizes for: ${missingSizes.map(i => i.name).join(', ')}. Continue without sizes?`)) {
//           // User confirmed to continue without sizes
//         } else {
//           return;
//         }
//       }
...
//               <div>
//                 <h3 className="text-lg font-semibold text-white">Dress Customization</h3>
//                 <p className="text-purple-100 text-sm mt-1">Customize your dress items (add/remove items, select sizes)</p>
//               </div>
//               <button
//                 onClick={handleAddDressItem}
//                 className="px-3 py-1 bg-white text-purple-600 rounded-md text-sm hover:bg-purple-50 transition-colors flex items-center gap-1"
//               >
...
//                     {/* Remove button */}
//                     <button
//                       onClick={() => handleRemoveDressItem(item.id)}
...
//                           type="text"
//                           value={item.name}
//                           onChange={(e) => handleUpdateDressItemName(item.id, e.target.value)}
//                           placeholder="e.g., Shirt, Pant, Tie, etc."
//                           className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
//                         />
//                       </div>
//                       <div>
...
//                           min="1"
//                           value={item.quantity}
//                           onChange={(e) => handleUpdateDressItemQuantity(item.id, e.target.value)}
//                           className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
//                         />
//                       </div>
//                       <div>
...
//               {dressCustomizations.length === 0 && (
//                 <div className="text-center py-8 text-gray-500">
//                   No dress items added. Click "Add Item" to add dress items.
//                 </div>
//               )}
//             </div>
//           </div>
...
//           <button
//             onClick={handleAccept}
//             disabled={submitting || !policyRead || !agreedToPolicy || (!isDressItem && !otp) || (isDressItem && dressCustomizations.length === 0)}
//             className="px-8 py-2 bg-gradient-to-r from-green-600 to-blue-600 text-white rounded-lg hover:from-green-700 hover:to-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 flex items-center gap-2 font-semibold"
//           >
//             {submitting ? (
//               <>
...
//             <span>→</span>
//             <div className="flex items-center gap-1">
//               <div className={`w-2 h-2 rounded-full ${isDressItem ? (dressCustomizations.length > 0 ? 'bg-green-500' : 'bg-gray-300') : (otp ? 'bg-green-500' : 'bg-gray-300')}`} />
//               <span>Details Provided</span>
//             </div>
//           </div>
//         </div>
...
const EmployeeAcceptance = () => {
  ...
  // Dress customization state - simplified to only name and quantity
  const [dressCustomizations, setDressCustomizations] = useState([]);
  ...
  const fetchDetails = async () => {
    ...
    if (result.success) {
      ...
      if (data.itemType?.toLowerCase().includes('dress') && data.dressItems && Object.keys(data.dressItems).length > 0) {
        const initialCustomizations = Object.entries(data.dressItems).map(([name, quantity], index) => {
          // Parse the name to extract base name (remove size info)
          let baseName = name;
          const match = name.match(/^(.*?)\s*\(([^)]+)\)$/);
          if (match) {
            baseName = match[1];
  ...
  const handleRemoveDressItem = (id) => {
    if (dressCustomizations.length === 1) {
      setError('At least one dress item is required');
      return;
    }
    setDressCustomizations(prev => prev.filter(item => item.id !== id));
  };
  ...
  const handleAccept = async () => {
    ...
    // OTP is required for ALL item types (both dress and non-dress)
    if (!otp || otp.trim() === '') {
      setError('Please enter the OTP');
      return;
    }
    ...
    // Validate dress items if it's a dress item
    if (details?.itemType?.toLowerCase().includes('dress')) {
      // Check if any item has no name
      const missingNames = dressCustomizations.filter(item => !item.name.trim());
      if (missingNames.length > 0) {
      ...
      // Check if dress items list is not empty
      if (dressCustomizations.length === 0) {
        setError('Please add at least one dress item');
        return;
      }
    }
    ...
    let result;
    if (details?.itemType?.toLowerCase().includes('dress')) {
      // Prepare customizations without color
      const customizations = dressCustomizations.map(item => ({
        name: item.name.trim(),
        quantity: item.quantity,
        size: '',  // Size is optional, can be added later
        color: '', // Not using color
        notes: item.notes
      }));
      result = await employeeItemApi.acceptDressWithCustomizations(token, otp, customizations);
    } else {
      result = await employeeItemApi.acceptItem(token, otp);
    }
    ...
    if (itemType.includes('id card')) {
      ...
      return {
        ...
        items: [
          ...
          'I am responsible for the maintenance of my assigned uniform.',
          'I understand that replacement uniform may be provided after 12 months of service.',
          'I will provide accurate quantity and specifications for my uniform.'
        ]
      };
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-8 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        {isDressItem && (
          <div className="bg-white rounded-2xl shadow-lg overflow-hidden mb-6">
            <div className="px-6 py-4 bg-gradient-to-r from-purple-600 to-pink-600 flex justify-between items-center">
              <div>
                <h3 className="text-lg font-semibold text-white">Dress Customization</h3>
                <p className="text-purple-100 text-sm mt-1">Customize your dress items (add/remove items)</p>
              </div>
              <button
                onClick={handleAddDressItem}
                className="px-3 py-1 bg-white text-purple-600 rounded-md text-sm hover:bg-purple-50 transition-colors flex items-center gap-1"
        ...
        {/* OTP Verification Section - For ALL item types */}
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden mb-6">
          <div className="px-6 py-4 bg-gradient-to-r from-blue-600 to-cyan-600">
            <h3 className="text-lg font-semibold text-white">OTP Verification</h3>
          </div>
        ...
        <div className="flex justify-end gap-3">
          ...
          <button
            onClick={handleAccept}
            disabled={submitting || !policyRead || !agreedToPolicy || !otp || (isDressItem && dressCustomizations.length === 0)}
            className="px-8 py-2 bg-gradient-to-r from-green-600 to-blue-600 text-white rounded-lg hover:from-green-700 hover:to-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 flex items-center gap-2 font-semibold"
          >
            {submitting ? (
              <>
        ...
        <div className="mt-6 text-center">
          <div className="inline-flex items-center gap-2 text-xs text-gray-400">
            ...
            <span>→</span>
            <div className="flex items-center gap-1">
              <div className={`w-2 h-2 rounded-full ${otp ? 'bg-green-500' : 'bg-gray-300'}`} />
              <span>OTP Entered</span>
            </div>
            <span>→</span>
            <div className="flex items-center gap-1">
              <div className={`w-2 h-2 rounded-full ${isDressItem ? (dressCustomizations.length > 0 && dressCustomizations.every(item => item.name.trim()) ? 'bg-green-500' : 'bg-gray-300') : 'bg-green-500'}`} />
