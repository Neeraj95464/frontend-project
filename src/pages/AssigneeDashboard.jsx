...
import { useEffect, useState, useMemo } from "react";
import { getAssigneePerformance, hasRole } from "../services/api";
import {
  BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip,
  ResponsiveContainer, Cell, LineChart, Line, Legend,
} from "recharts";
...
const AssigneeDashboard = () => {
  ...
  // ✅ NEW: Department filter state for SUPER_ADMIN
  const [departmentFilter, setDepartmentFilter] = useState("ALL"); // "ALL", "HR", "EXCEPT_HR"
  const isSuperAdmin = hasRole("SUPER_ADMIN");

  // Generate year options: current year back 3 years
  const yearOptions = Array.from({ length: 4 }, (_, i) => defaultYear - i);

  // ✅ UPDATED: Fetch overall stats with department filter
  useEffect(() => {
    const fetchOverall = async () => {
      try {
        setLoading(true);
        const filterParam = isSuperAdmin && departmentFilter !== "ALL" ? departmentFilter : undefined;
        const result = await getAssigneePerformance(undefined, undefined, filterParam);
        const overall = (result?.overallStats || []).sort((a, b) => b.totalTickets - a.totalTickets);
        setOverallStats(overall);
      } catch (e) {
        console.error("Dashboard error:", e);
  ...
  // ✅ UPDATED: Fetch monthly stats with department filter
  useEffect(() => {
    const fetchMonthly = async () => {
      setMonthlyLoading(true);
      try {
        const filterParam = isSuperAdmin && departmentFilter !== "ALL" ? departmentFilter : undefined;
        const result = await getAssigneePerformance(selectedMonth, selectedYear, filterParam);
        setMonthlyStats(result?.monthlyStats || []);
      } catch (e) {
        console.error("Monthly fetch error:", e);
      } finally {
    ...
    fetchMonthly();
  }, [selectedMonth, selectedYear, departmentFilter, isSuperAdmin]);
  ...
  // Bar chart data — monthly stats sorted descending by closedTickets
  const barChartData = useMemo(() => {
    return [...monthlyStats].sort((a, b) => b.closedTickets - a.closedTickets);
  }, [monthlyStats]);
  ...
  const getFilterLabel = () => {
    ...
    switch (departmentFilter) {
      ...
      case "EXCEPT_HR": return " (All Except HR)";
      ...
      <div className="lg:ml-48 bg-gray-50 min-h-screen">
        ...
        <div className="dash-root">
          <div style={{ maxWidth: 1400, margin: "0 auto" }}>
            ...
            <div style={{ marginBottom: 28 }}>
              <h1 style={{ margin: 0, fontSize: 28, fontWeight: 800, color: "#F1F5F9", letterSpacing: "-0.5px" }}>
                Assignee Performance{getFilterLabel()}
              </h1>
              <p style={{ margin: "6px 0 0", fontSize: 13, color: "#64748B" }}>
                Comprehensive overview of support agent metrics and ticket resolution
              </p>
            ...
            <div style={{ display: "flex", alignItems: "center", flexWrap: "wrap", gap: 16, marginBottom: 20 }}>
              ...
              {/* ✅ NEW: Department Filter for SUPER_ADMIN */}
              {isSuperAdmin && (
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginLeft: "auto" }}>
                  ...
                  <div style={{ position: "relative" }}>
                    ...
                    >
                      ...
                      <option value="EXCEPT_HR">⚙️ All Except HR</option>
                    </select>
                  </div>
                </div>
              )}

              {/* Month/Year pickers (visible in chart tab) */}
              {activeTab === "chart" && (
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ fontSize: 12, color: "#64748B", fontWeight: 600 }}>Period:</span>
                  <div style={{ position: "relative" }}>
