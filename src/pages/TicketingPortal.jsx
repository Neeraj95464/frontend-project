...
//   useEffect(() => {
//     let role = "user";
//     if (hasRole("ADMIN") || hasRole("HR_ADMIN") || hasRole("SUPER_ADMIN")
//       || hasRole("EXECUTIVE") || hasRole("MANAGER")) role = "admin";
//     setUserRole(role);
//   }, []);
...
export default function TicketingPortal() {
  ...
  // Measure toolbar height so table header sticks exactly below it
  useEffect(() => {
  ...
  return (
    <div className="lg:ml-48 bg-slate-50 min-h-screen flex flex-col font-sans">
      ...
      {/* ═══ TOOLBAR ═══ */}
      <div ref={toolbarRef} className="sticky top-0 z-30 bg-white border-b border-gray-200 shadow-sm">
        <div className="flex flex-wrap items-center gap-1.5 px-3 py-1.5">
      ...
      {/* ═══ TABLE ═══ */}
      {/*
        KEY CHANGE: the table wrapper is now a fixed-height scrollable container
        that fills the remaining viewport. The thead uses `sticky top-0` which
        positions it relative to this scrolling ancestor — not the page — so it
        stays pinned inside the table while the toolbar stays pinned at the top
        of the page. No z-index conflicts, no double-scroll.
      */}
      <div
        className="flex-1 overflow-auto tp-scrollbar"
